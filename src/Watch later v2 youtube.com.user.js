// ==UserScript==
// @name        Watch later v2 youtube.com
 ...
 * https://github.com/WorldThirteen/youtube-watch-later-shortcut-ext/blob/23edaac175c0689149df1a6824648a395a7b9885/chrome/background.js#L14
 */
/**
 * mobile web version
 * research test page
...
document.addEventListener('keydown', function (e) {
    ...
    if (e.key == 'q' || e.key == 'e') {
        ...
        let actions = null;
        let setVideoId = "E21CAF7EC9CCB773"
        ...
        if (videoId) {
            ...
            console.log(actions)
            apiRestful_playlist(actions, 'WL').then(res => {
                if (res.status == 200) {
                    flash('green', element)
                } else {
                    flash('red', element)
...
function getVideoId(element) {
    if (!element) {
        element = elementMouseHover();
    }

    let videoId = null
    if (videoId == null) {
        try {
            videoId = element.querySelector('a[href *= "/watch?v="]').href.match(/watch\?v=([^=&\?]+)&?/)[1];
    ...
    if (videoId == null) {
        try {
            videoId = element.querySelector('a[href *= "/shorts/"]').href.match(/shorts\/([^\/\?]+)\/?/)[1];
    ...
    if (videoId == null) {
        try {
            videoId = element.innerHTML.match(/watch\?v=([^=&\?]+)&?/)[0];
        } catch (error) {
    ...
    if (videoId == null) {
        try {
            videoId = element.innerHTML.match(/shorts\/([^\/\?]+)\/?/)[0];
        } catch (error) {
...
// async function apiRestful_get_setVideoId(videoId) {
//     apiRestful_playlist(actions, 'WL').then(res => {
//         if (res.status == 200) {
//             return res.json();
//         }
...
// apiRestful_playlist(actions, 'WL');